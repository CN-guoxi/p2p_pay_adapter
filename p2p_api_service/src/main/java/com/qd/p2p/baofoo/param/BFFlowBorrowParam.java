...
@Adapted(value = AdaptePlatform.BAOFOO, adaptedClass = FlowBorrowParam.class)
public class BFFlowBorrowParam extends AbstractRequestParam implements ParameterAdapter<FlowBorrowParam> {
    /**
     * 商户号
     */
    ...
    @Override
    public void adapter(FlowBorrowParam param) {
        this.merchantId = ConfigurationConsts.MERCHANT_ID;
        this.actionType = 3; // 投标
        ...
        /* 投标用户 */
        List<FlowBorrowParam.TenderUser> tenderUsers = param.getTenderUsers();
        Action action;
        List<Action> actions = Lists.newArrayList();
        for (FlowBorrowParam.TenderUser tenderUser : tenderUsers) {
            action = new Action();

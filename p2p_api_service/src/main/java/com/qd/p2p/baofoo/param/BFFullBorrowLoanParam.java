...
@Adapted(value = AdaptePlatform.BAOFOO, adaptedClass = FullBorrowLoanParam.class)
public class BFFullBorrowLoanParam extends AbstractRequestParam implements ParameterAdapter<FullBorrowLoanParam> {
    /**
     * 商户号
     */
    ...
    @Override
    public void adapter(FullBorrowLoanParam param) {
        this.merchantId = ConfigurationConsts.MERCHANT_ID;
        this.actionType = 2; // 满标放款
        ...
        action.setUserId(String.valueOf(param.getBorrowUserId())); // 借款人用户ID
        action.setUserName(param.getBorrowUserName()); // 借款人用户ID
        action.setAmount(param.getAmount()); // 借款金额
        action.setIsVoucher("0"); // 借款人
